...
class MainController extends Controller
{
    ...
    public function quiz($slug){
        $quiz = Quiz::whereSlug($slug)->with('questions.my_option','my_result')->first() ?? abort(404, 'Quiz Bulunamadı');
    ...
    public function result(Request $request, $slug){
        ...
        foreach($quiz->questions as $question){
            Answer::create([
                ...
                'option'=>$request->post($question->id)
            ]);

            if($question->correct_option===$request->post($question->id)){
                $correct+=1;
            }
        }
